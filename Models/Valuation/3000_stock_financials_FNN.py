...
# Dropping cat_cols for consistency with LR models
df = df.drop(columns = cat_cols)

# Assume df is your dataframe
X = df.drop(columns=['commonstockvalue']).values.astype(np.float32)  # Features
y = df['commonstockvalue'].values.reshape(-1, 1).astype(np.float32)  # y
...
thisFile = "3000_stock_financials_FNN"
