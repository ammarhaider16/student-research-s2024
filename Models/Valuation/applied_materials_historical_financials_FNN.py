...
df = pd.read_csv("../Datasets/Valuation/applied_materials_historical_financials.csv")
...
X = df.drop(columns=['Adj Close']).values.astype(np.float32)  # Features
y = df['Adj Close'].values.reshape(-1, 1).astype(np.float32)  # y
...
thisFile = "applied_materials_historical_financials_FNN"
...
handleAddValuationNormalizedMSE("FNN",thisFile,modelOneOutputList,modelTwoOutputList,y_val)