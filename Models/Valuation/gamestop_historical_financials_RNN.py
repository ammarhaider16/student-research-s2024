...
X = scaler.fit_transform(df.drop(columns=['adjclose_price']).values)
y = df['adjclose_price'].values  # Assuming target does not need scaling
...
mse_tf = np.mean(np.square(y_test - y_pred_tf))
...
y_pred_th = y_pred_tensor.numpy()
mse_pytorch = mean_squared_error(y_test, y_pred_th)
